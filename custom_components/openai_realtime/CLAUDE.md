...
**Pipeline stage semantics** (each stage maps to a distinct Realtime API mechanism so the stages compose without double model turns):
  ...
  - The conversation agent (`send_text`) adds a user item + a **text-only** `response.create` (`output_modalities: ["text"]`) and assembles the reply from `response.output_text.delta`, finishing on `response.done`. Conversation responses must never produce audio — their media-track playout would leak into the TTS stage's capture and the user would hear the reply twice. Conversation history lives server-side in the realtime session, so multi-turn context persists while the call is up.
